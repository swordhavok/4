...
setup_general() {
    ...
    # create backup directory for resolv.conf if it doesn't exist
    ...
    if ! cp /etc/resolv.conf "${backup_dir}/etc/resolv.conf.backup"; then
...
## Restart Tor service every 30 seconds
restart_tor() {
    while true
    do
        sleep 30
        info "Restarting Tor service to change IP address"
        systemctl restart tor.service
        sleep 10 # wait for Tor service to restart
        check_status
    done
}
...
stop() {
    ...
    if systemctl is-active tor.service >/dev/null 2>&1; then
        ...
        if hash resolvconf 2>/dev/null; then
            ...
            cp "${backup_dir}/etc/resolv.conf.backup" /etc/resolv.conf
        fi
...
usage() {
    ...
    printf "%s\\n" "-k, --kill-restart  stop the restart tor loop"
